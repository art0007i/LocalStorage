...
#nullable enable
    ...
    {
        Log = base.Log;
        RecordPath = Config.Bind("General", "RecordPath", "",
            "The path in which records will be stored. Changing this setting requires a game restart to apply.");
        DataPath = Config.Bind("General", "DataPath", "",
            "The path in which item data will be stored. Changing this setting requires a game restart to apply.");
        ...
        BepisResoniteWrapper.ResoniteHooks.OnEngineReady += () =>
        {
            ...
            Log.LogDebug("Creating local storage directories");
            ...
            if (!Directory.Exists(REC_PATH))
            {
                ...
                {
                    HIDE_LOCAL = true;
                    Log.LogError("A critical error has occured while creating record directory");
                    Log.LogError(e);
                }
            }
            ...
            if (!Directory.Exists(DATA_PATH))
            {
                ...
                {
                    HIDE_LOCAL = true;
                    Log.LogError("A critical error has occured while creating data directory");
                    Log.LogError(e);
                }
            }

            Log.LogInfo("Initalized LocalStorage\nData Path: " + DATA_PATH + "\nRecord Path: " + REC_PATH);
            HIDE_LOCAL = false;
        };
    }
    ...
    class InventoryPatch
    {
        ...
        {
            if (!HIDE_LOCAL && __instance?.Engine?.Cloud?.CurrentUser == null &&
                __instance?.CurrentDirectory?.OwnerId != LOCAL_OWNER && __instance.World.IsUserspace())
            {
            ...
            else if (__instance?.CurrentDirectory?.OwnerId == LOCAL_OWNER &&
                     __instance?.Engine?.Cloud?.CurrentUser == null && __instance.World.IsUserspace())
            {
        ...
        public static void GenerationPostfix(InventoryBrowser __instance, UIBuilder __result, ref GridLayout folders)
        {
            if (!HIDE_LOCAL && __instance.CurrentDirectory == null && __instance.World.IsUserspace())
            {
                ...
                var openFunc = (ButtonEventHandler<string>)AccessTools.Method(typeof(InventoryBrowser), "OpenInventory")
                    .CreateDelegate(typeof(ButtonEventHandler<string>), __instance);
    ...
    class RecordDirectoryPatch
    {
        ...
        public static bool LoadFrom(string ownerId, string path, ref Task __result, RecordDirectory __instance)
        {
            if (ownerId == LOCAL_OWNER)
            {
                __result = Task.Run(() =>
                {
                    ...
                    {
                        var dirRec = SkyFrost.Base.RecordHelper.CreateForDirectory<Record>(LOCAL_OWNER, path,
        ...
        [HarmonyPrefix]
        public static bool AddItem(string name, Uri objectData, Uri thumbnail, IEnumerable<string> tags,
            ref Record __result, RecordDirectory __instance)
        {
            ...
            if (__instance.OwnerId == LOCAL_OWNER)
            {
                ...
                Log.LogDebug("SAVING " + objectData.ToString());
                ...
                */
                ...
                var rec = SkyFrost.Base.RecordHelper.CreateForObject<Record>(name, __instance.OwnerId, fileLocalPath,
                    thumbLocalPath);
        ...
        [HarmonyPrefix]
        public static bool AddSubdirectory(string name, bool dummyOnly, RecordDirectory __instance,
            ref RecordDirectory __result)
        {
            if (__instance.OwnerId == LOCAL_OWNER)
            {
                var fixedPath = __instance.ChildRecordPath.Replace('\\', '/');
                ...
                var rec = SkyFrost.Base.RecordHelper.CreateForDirectory<Record>(LOCAL_OWNER, __instance.ChildRecordPath,
                    name);
                if (!dummyOnly)
                {
                ...
                (AccessTools.Field(typeof(RecordDirectory), "subdirectories").GetValue(__instance) as
                    List<RecordDirectory>).Add(ret);
                __result = ret;
                return false;
            }
        ...
        public static bool AddLinkAsync(string name, Uri target, RecordDirectory __instance, ref Task<Record> __result)
        {
            if (__instance.OwnerId == LOCAL_OWNER)
            {
                var fixedPath = __instance.ChildRecordPath.Replace('\\', '/');
                Record record =
                    SkyFrost.Base.RecordHelper.CreateForLink<Record>(name, __instance.OwnerId, target.ToString(), null);
        ...
        public static bool DeleteItem(Record record, RecordDirectory __instance, ref bool __result)
        {
            if (__instance.OwnerId == LOCAL_OWNER)
            {
                var test = (AccessTools.Field(typeof(RecordDirectory), "records").GetValue(__instance) as List<Record>)
                    .Remove(record);
                if (test)
                {
        ...
        public static async Task RecursiveDelete(RecordDirectory dir)
        {
            ...
            {
                File.Delete(Path.Combine(REC_PATH, dir.LinkRecord.Path.Replace('\\', '/'), dir.LinkRecord.Name) +
                            ".json");
            }
        }
    }
    ...
    [HarmonyPatch(typeof(DataTreeConverter), nameof(DataTreeConverter.Load),
